#!/usr/bin/python
# -*- coding: UTF-8 -*-

import copy
import PrintCipher96_model
import time
import os

if __name__ == "__main__":

    cd = dict()

    cd["cipher_name"] = "PrintCipher96_max_round"

    cd["cipher_size"] = 96
    cd["sbox_size"] = 3
    cd["sbox_num"] = 32

    cd["mode"] = 1

    folder = cd["cipher_name"] + "_mode_{}".format(cd["mode"])

    if not os.path.exists(folder):
        os.mkdir(folder)

    search_space = list()
    for i in range(0, 32):
        for i_value in range(1, 8):
            b1 = [0 for bii in range(0, cd["cipher_size"])]
            for ii in range(0, 3):
                b1[3 * i + ii] = (i_value >> ii) & 0x1
            b = PrintCipher96_model.perm_wire_value(copy.deepcopy(b1))
            for j in range(0, 32):
                for j_value in range(1, 8):
                    e1 = [0 for eii in range(0, cd["cipher_size"])]
                    for jj in range(0, 3):
                        e1[3 * j + jj] = (j_value >> jj) & 0x1
                    search_space.append([copy.deepcopy(b), copy.deepcopy(e1)])

    round_i = 5
    cd["record_file"] = folder + "////" + cd["cipher_name"] + "_record_mode{}.txt".format(cd["mode"])
    cd["time_record"] = folder + "////" + cd["cipher_name"] + "_time_record_mode{}.txt".format(cd["mode"])
    total_search = len(search_space)
    ttt1 = time.time()

    cd["solve_file"] = folder + "////" + cd["cipher_name"] + "_round{}.stp".format(round_i)
    t1 = time.time()
    search_count = 0
    distinguish_count = 0
    for ss in search_space:
        cd["b1"] = copy.deepcopy(ss[0])
        cd["e1"] = copy.deepcopy(ss[1])
        mode = [cd["mode"], [0, round_i]]
        t11 = time.time()
        search_count += 1
        PrintCipher96_model.model_build(cd, mode)
        flag = PrintCipher96_model.solver(cd["solve_file"])
        t22 = time.time()
        print(t22 - t11)
        print("testing: search_count = {}, total_search = {}".format(search_count, total_search))
        if flag:
            distinguish_count += 1
            rf = open(cd["record_file"], "a")
            rf.write("*" * 20)
            rf.write("{} 5round impossible distinguish found\n".format(distinguish_count))
            rf.write("when the values:\n")
            rf.write("b1 = {}\n".format(str(cd["b1"])))
            rf.write("e1 = {}\n".format(str(cd["e1"])))
            rf.close()
    t2 = time.time()