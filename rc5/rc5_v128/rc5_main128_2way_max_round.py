...
if __name__ == "__main__":
    ...
    cd["cipher_name"] = "rc5_128_{}way_max_round".format(cd["mul"])
    ...
    search_space = list()
    for i in range(0, 64):
        x = [0 for xii in range(0, 128)]
        x[i] = 1
        x[i + 64] = 1
        for j in range(0, 128):
            y = [0 for yii in range(0, 128)]
            y[j] = 1
    ...
    for ss in search_space:
        ...
        t11 = time.time()
        search_count += 1
        rc5_model128.model_build(cd, mode)
        flag = rc5_model128.solver(cd["solve_file"])
        t22 = time.time()
        print(t22 - t11)
